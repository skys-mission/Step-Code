...
import { parseCommandArgs } from "@step-harness/agent-core";
import { APP_NAME, stripBom } from "@step-harness/coding-agent";
...
export async function editInExternalEditor(options: ExternalEditorOptions): Promise<ExternalEditorResult> {
	const safeAppName = APP_NAME.replace(/[^A-Za-z0-9._-]/g, "-");
	const directory = mkdtempSync(join(tmpdir(), `${safeAppName}-editor-`));
	...
	try {
		...
		const exitCode = await new Promise<number | null>((resolve) => {
			// Windows needs a shell for .cmd/.bat editors; retain the command's quoting there.
			const child =
				process.platform === "win32"
					? spawn(`${options.command} "${filePath}"`, { stdio: "inherit", shell: true })
